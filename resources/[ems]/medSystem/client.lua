...
local md = Config.Declared
	...
	if GetDistanceBetweenCoords(x,y,z,a,b,c,false) < 2 then
		timer = Config.Timer
...
function CheckBP(closestPlayer)
    ...
    if closestPlayer == -1 or closestDistance > 1.7 then
        ...
        TriggerServerEvent('medSystem:checkBP', GetPlayerServerId(closestPlayer))
    end
end
...
RegisterNetEvent('medSystem:checkBP')
AddEventHandler('medSystem:checkBP', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
