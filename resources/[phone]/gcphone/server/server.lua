...
AddEventHandler('gcPhone:useSimCard', function(source, identifier)
    ...
    until id == nil
    MySQL.Async.insert("UPDATE users SET phonenumber = @myPhoneNumber WHERE identifier = @identifier", { 
...
function getNumberPhone(identifier)
    local result = MySQL.Sync.fetchAll("SELECT users.phonenumber FROM users WHERE users.identifier = @identifier", {
...
function getIdentifierByPhoneNumber(phonenumber) 
    local result = MySQL.Sync.fetchAll("SELECT users.identifier FROM users WHERE users.phonenumber = @phonenumber", {
...
function getMessages(identifier)
    local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.phonenumber", {
         ['@identifier'] = identifier
    })
    return result
    --return MySQLQueryTimeStamp("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.phonenumber", {['@identifier'] = identifier})
end
...
function addMessage(source, identifier, phonenumber, message, gps_data)
    local sourcePlayer = tonumber(source)
    ...
    if otherIdentifier ~= nil then 
        local tomess = _internalAddMessage(myPhone, phonenumber, message, 0)
    ...
    local memess = _internalAddMessage(phonenumber, myPhone, message, 1)
...
function deleteAllMessageFromPhoneNumber(source, identifier, phonenumber)
    local source = source
    local identifier = identifier
    local mePhoneNumber = getNumberPhone(identifier)
    MySQL.Sync.execute("DELETE FROM phone_messages WHERE `receiver` = @mePhoneNumber and `transmitter` = @phonenumber", {['@mePhoneNumber'] = mePhoneNumber,['@phonenumber'] = phonenumber})
end
...
AddEventHandler('gcPhone:register_FixePhone', function(phonenumber, coords)
	Config.FixePhone[phonenumber] = {name = _U('phone_booth'), coords = {x = coords.x, y = coords.y, z = coords.z}}
	TriggerClientEvent('gcPhone:register_FixePhone', -1, phonenumber, Config.FixePhone[phonenumber])
end)

RegisterServerEvent('gcPhone:internal_startCall')
AddEventHandler('gcPhone:internal_startCall', function(source, phonenumber, rtcOffer, extraData)
    if Config.FixePhone[phonenumber] ~= nil then
        onCallFixePhone(source, phonenumber, rtcOffer, extraData)
        return
    end
    ...
    local destPlayer = getIdentifierByPhoneNumber(phonenumber)
...
AddEventHandler('gcPhone:startCall', function(phonenumber, rtcOffer, extraData)
    local _source = source
    TriggerEvent('gcPhone:internal_startCall',_source, phonenumber, rtcOffer, extraData)
end)
...
function onCallFixePhone (source, phonenumber, rtcOffer, extraData)
    local indexCall = lastIndexCall
    lastIndexCall = lastIndexCall + 1
    ...
    AppelsEnCours[indexCall] = {
        ...
        rtcOffer = rtcOffer,
        extraData = extraData,
        coords = Config.FixePhone[phonenumber].coords
    }
...
--[[
RegisterServerEvent('addphoto')
AddEventHandler('addphoto', function(url2)
    local _source = source
    local sourcePlayer = tonumber(_source)
	xPlayer = ESX.GetPlayerFromId(_source)
   
        xPlayer.addInventoryItem('photo',1 ,{  photo = url2  , description = url2})
end)
