...
RegisterNetEvent("nh-context:mechanicMenu", function()
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
    TriggerEvent("nh-context:createMenu", {
        {
            header = "🧑‍🔬 Mechanic Menu",
			context = "This Menu is for Mechanic",
        },
     	{
            header = "🚗 Mechanic Interaction",
            context = "Mechanic Interaction for Mechanic",
            event = "nh-context:MechanicAction"
         
        },
		{
            header = "🧱 Object Interaction",
            context = "Object Interaction for Mechanic",
            event = "nh-context:mechanicObject"
...
RegisterNetEvent('nh-context:MechanicAction', function(id, number)
	local action = exports["nh-context"]:ContextMenu({
		...
		--{label = _U('place_objects'), value = 'object_spawner'}
        {
            ...
            event = "nh-context:mechanicMenu"
        },
        {
            header ='💳 ' .. _U('billing'),
			context = "Create Billing",
			args = {"billing"}
        },
		{
            header ='⛮ Force Open Vehicle',
			context = "Force Open Nearby Vehicle",
			args = {"hijack_vehicle"}
        },
		{
            header = '🔨 ' ..   _U('repair'),
			context = "Repair Nearby Vehicle",
			args ={"fix_vehicle"}
        },
		{
            header = '🧽 ' ..  _U('clean'),
			context = "Clean Nearby Vehicle",
			args ={"clean_vehicle"}
        },
		{
            header ='🚗 ' .._U('imp_veh'),
			context = "Impound Nearby Vehicle",
			args = {"del_vehicle"}
        },
		{
            header ='🚜 ' .. _U('flat_bed'),
			context = "Flatbed Nearby Vehicle",
			args = {"dep_vehicle"}
        },
		...
		elseif action == 'fix_vehicle' then
			...
			local vehicleRepairkit = exports.ox_inventory:Search('count', 'fixtool')
			
			if vehicleRepairkit > 0 then
			if exports["k5_skillcheck"]:skillCheck("easy") then
				Citizen.Wait(100)
	if lib.progressCircle({
		duration = 20000,
	...
	ESX.ShowNotification('Need Vehicle Repairkit')
end
			else
				ESX.ShowNotification(_U('no_vehicle_nearby'))
			end
...
RegisterNetEvent('nh-context:mechanicObject', function(id, number)
	local action = exports["nh-context"]:ContextMenu({
		...
		{
            header ='🧱 ' .. _U('toolbox'),
			context = "Create Chest",
			args = {"prop_toolchest_01"}
        },
	   })
...
function OpenMechanicActionsMenu()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
		local action = exports["nh-context"]:ContextMenu({
			...
			{
				header = "👚 "..  _U('work_wear'),
				context = "Select Work Wear",
				args = {grade}
			 
			},
			{
				header = "👘 "..  _U('civ_wear'),
				context = "Select Citizen Wear",
				args = {"cloakroom2"}
			 
			},
			{
				header = "🗄️ "..  _U('deposit_stock'),
				context = "Object Interaction for Mechanic",
				args = {"put_stock"}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(action, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
					else
		...
		elseif action == 'cloakroom2' then
			--menu.close()
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				TriggerEvent('skinchanger:loadSkin', skin)
			end)

		elseif Config.OxInventory and action == 'put_stock'  then
			exports.ox_inventory:openInventory('stash', 'society_mechanic')
			return ESX.UI.Menu.CloseAll()
		elseif Config.OxInventory and action == 'get_stock' then
			--OpenPutStocksMenu()
			exports.ox_inventory:openInventory('stash', {id='mechanic_boss'})
			

		elseif action == 'get_stock' then
			OpenGetStocksMenu()
		elseif action == 'boss_actions' then
...
RegisterCommand('mechanicMenu', function()
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			...
			TriggerEvent('nh-context:mechanicMenu')
		end
end, false)
