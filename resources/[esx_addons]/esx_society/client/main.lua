	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			local action = exports["nh-context"]:ContextMenu({
				...
				{
					header = "Boss Menu ",
					context = 'Society Money : $'.. socMoney
				},
				{
...
RegisterNetEvent("nh-context:ManageMent", function(society)
	...
	local actions = exports["nh-context"]:ContextMenu({
		...
		{
			header = "Employee Management Menu",
		},
		{
			header = "🧑‍🤝‍🧑"..  _U('employee_list'),
			context = "Check Employee List",
			args = {"employee_list"},
		},

		{
			header ='🔍 ' .. _U('recruit'),
			context = "Hire New Citizen",
			args = {"recruit"}
		},			  
	})
