	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
			...
			for hospitalNum,hospital in pairs(Config.Hospitals) do
				...
				for k,v in ipairs(hospital.StorageActions) do
					local distance = #(playerCoords - v)
					...
					if distance < Config.DrawDistance then
						...
						if distance < Config.Marker.x then
							isInMarker, currentHospital, currentPart, currentPartNum = true, hospitalNum, 'StorageActions', k
						end
					end
				end
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	...
	elseif part == 'AmbulanceActions' then
		...
		CurrentActionData = {}
	elseif part == 'StorageActions' then
		CurrentAction = part
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'AmbulanceActions' then
					...
					OpenCloakroomMenu()
					elseif CurrentAction == 'StorageActions' then
						OpenStorageMenu()
...
RegisterNetEvent("nh-context:emsMenu", function()
    TriggerEvent("nh-context:createMenu", {
        {
            header = "🏥 Ems Menu",
			context = "This goes to a Ems Menu",
        },
        {
            header = "👨‍⚕️ Citizen Interaction",
            context = "This goes to a Ems Interaction",
            event = "nh-context:emsCitizen",
	...
	--[[	{
            ...
            event = "nh-context:emsVehicle"
         
        },]]
		{
            header = "📱 MDT/Tablet",
            context = "This goes to Record of Patient and Distress",
            event = "openTabletEMS"
...
RegisterNetEvent('openTabletEMS', function()
	if  ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		TriggerServerEvent('OpenMDT2')
	end
	
end)

RegisterNetEvent('nh-context:emsCitizen', function(id, number)
	local action = exports["nh-context"]:ContextMenu({
...

        {
            ...
            event = "nh-context:emsMenu"
        },
      
		{
            header ='🩹 '.. _U('ems_menu_small'),
			context = "Perform Treat Small in Nearby Citizent",
			args = {"small"}
        },
		{
            header ='❤️ '.. _U('ems_menu_big'),
			context = "Perform Treat Big in Nearby Citizent",
			args = {"big"}
        },
		{
            header ='👨‍⚕️'.. _U('ems_menu_revive'),
			context = "Perform CPR in Nearby Citizent",
			args = {"revive"}
        },
		{
            header = "❤️‍🩹 Check BP",
            context = "Checking Patient Vitals",
            event = "medSystem:checkBP",
			
         
        },
		
		--[[{
            header ='🚑 '.. _U('ems_menu_putincar'),
			context = "Put Nearby Citizent in Vehicle",
			args = {"put_in_vehicle"}
        },]]
		{
            header ='🔎 '.. _U('ems_menu_search'),
			context = "Searh Nearby Citizent",
			args = {"search"}
        },
...
RegisterCommand("ambulance", function(src)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' and not ESX.PlayerData.dead then
		...
		TriggerEvent('nh-context:emsMenu')
	end
end)
...
function OpenCloakroomMenu()
	...
	local action = exports["nh-context"]:ContextMenu({
        ...
        {
            header = "🚑 EMS Menu",
			context = "EMS Locker Room Menu",
        },
        {
            header = "🧑‍🤝‍🧑"..  _U('ems_clothes_civil'),            
			context = "Get your Clothes",
            args = {"citizen_wear"},
        },

		{
            header ='👔 ' .. _U('ems_clothes_ems'),
			context = "Uniform on Duty",
			args = {"ambulance_wear"}
        },
	...
	elseif action == 'ambulance_wear' then
		ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
			if skin.sex == 0 then
